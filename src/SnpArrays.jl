...
export SnpArray, SnpArrayBM, summarysnps, grm, _grmbm, impute!, summarize
...
function SnpArray(plinkFile::AbstractString)
  ...
  if bits(bedheader[1]) == "01101100" && bits(bedheader[2]) == "00011011"
    ...
    if bits(bedheader[3]) == "00000001"
      ...
      # individual-major
      snpbits = Mmap.mmap(fid, BitArray{3}, (2, 4ceil(Int, 0.25nsnp), nper), 3; grow=false)
...
function SnpArrayBM(plinkFile::AbstractString)
  ...
  if bits(bedheader[1]) == "01101100" && bits(bedheader[2]) == "00011011"
    if bits(bedheader[3]) == "00000001"
      snpbits = Mmap.mmap(fid, BitArray{3}, (2, 4ceil(Int, 0.25nper), nsnp), 3)
      A = !slice(snpbits, :, 1:nper, :)
    else
      snpbits = Mmap.mmap(fid, BitArray{3}, (2, 4ceil(Int, 0.25nsnp), nper), 3)
...
typealias SnpLike{N} Union{
    SnpArray{N}, SnpArrayBM{N},
